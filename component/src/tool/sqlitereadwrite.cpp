 ...
 * @LastEditTime: 2023-10-16 17:15:11
...
#ifdef UseSqlcipher
    mDB = QSqlDatabase::addDatabase(QSqlcipherDriver, SqlcipherDriver);
#else
    mDB = QSqlDatabase::addDatabase(QSqliteDriver,SqliteDriver);
#endif
...
{ // 相对于mysql没有相关命令,只能是文件操作: 创建这个文件
    if (!source.endsWith(SqliteFileSuffix)) return false;
