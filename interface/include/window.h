...
struct PlateImageInfo
{
    ...
    QColor getHoleColor(int x, int y) const {
        for(auto image: images) {
            if (image.view.hole.getCoordinate() == QPoint(x,y))
                return image.holeColor();
        }
        return QColor();
    }

    QColor getHoleColor(const QPoint& hole) const {
        for(auto image: images) {
            if (image.view.hole.getCoordinate() == hole)
                return image.view.hole.color;
        }
        return QColor();
    }
...
struct DataPatternHoleInfo
{
    ...
    QColor getHoleColor() const { // 找到孔的眼色便于孔的颜色分组

        return info.getHoleColor(hole);
    }
};
